...
def theta_generate(Theta_1_size, Theta_2_size, Theta_3_size, n):
    ...
    folder = f'{BASE_DIR}/genetic_training_data'
    ...
    for dataset in range(n):
        theta_init.create_file(Theta_1_size, Theta_2_size, Theta_3_size, file_name = f"{BASE_DIR}/genetic_training_data/nn_theta_set_{dataset}.npz", init_type = "logistic")

def theta_review(theta_set):

    """Display purposes"""

    theta = np.load(f"{BASE_DIR}/genetic_elite_data/nn_theta_set_0.npz")
    # theta = np.load(f"{BASE_DIR}/manuel_best_copy/Nov 27 depth 6 unbeatable")
    # theta = np.load(f"{BASE_DIR}/genetic_training_data/nn_theta_set_{theta_set}.npz")
    print(theta['Theta1'])
    print(theta['Theta2'])
    print(theta['Theta3'])
...
def multi_tournament_phase_1(heats, survivor_num, nn_depth, random_depth):
    ...
    population = len(os.listdir(f'{BASE_DIR}/genetic_training_data'))
    ...
    folder = f'{BASE_DIR}/genetic_elite_data'
    for filename in os.listdir(folder):
        file_path = os.path.join(folder, filename)
        try:
            if os.path.isfile(file_path) or os.path.islink(file_path):
    ...
    shutil.copyfile(f'{BASE_DIR}/genetic_training_data/nn_theta_set_{strongest}.npz', 
                    f'{BASE_DIR}/genetic_elite_data/nn_theta_set_0.npz'
    )
    ...
    for player in death:
        os.remove(f"{BASE_DIR}/genetic_training_data/nn_theta_set_{player}.npz")
    
    survivors.sort()
    n = 0
    for player in survivors:
        os.rename(f"{BASE_DIR}/genetic_training_data/nn_theta_set_{player}.npz", 
                  f"{BASE_DIR}/genetic_training_data/nn_theta_set_{n}.npz")
        n += 1
    
    folder = f'{BASE_DIR}/genetic_parent_data'
    for filename in os.listdir(folder):
        file_path = os.path.join(folder, filename)
        try:
            if os.path.isfile(file_path) or os.path.islink(file_path):
    ...
    source = f'{BASE_DIR}/genetic_training_data/'
    destination = f'{BASE_DIR}/genetic_parent_data/'
...
def multi_tournament_phase_2(heats, survivor_num, nn_depth, random_depth):
    ...
    folder = f'{BASE_DIR}/genetic_trainer_data'
    for filename in os.listdir(folder):
        file_path = os.path.join(folder, filename)
        try:
            if os.path.isfile(file_path) or os.path.islink(file_path):
    ...
    for player in trainer:
        shutil.copyfile(f'{BASE_DIR}/genetic_training_data/nn_theta_set_{strongest}.npz', 
                    f'{BASE_DIR}/genetic_trainer_data/nn_theta_set_{n}.npz')
        n += 1
...
def multi_reproduction(goal_population, population):
    ...
    while current_population < goal_population:
        ...
        np.savez(f'{BASE_DIR}/genetic_training_data/nn_theta_set_{current_population}.npz', Theta1 = theta_1, Theta2 = theta_2, Theta3 = theta_3)
        current_population += 1

    shutil.copyfile(f'{BASE_DIR}/genetic_elite_data/nn_theta_set_0.npz', 
                    f'{BASE_DIR}/genetic_training_data/nn_theta_set_0.npz'
    )

def mutate(mutation_rate):

    allfiles = os.listdir(f'{BASE_DIR}/genetic_training_data')
    population = len(allfiles)
...
def genetic_algorithm(survivor_num, generations, heats, mutation_rate, nn_depth, random_depth, training_phase = 1, heat_size = 1):
    ...
    for i in range(generations):
        ...
        shutil.copyfile(
            f"{BASE_DIR}/genetic_training_data/nn_theta_set_0.npz", 
            f"{BASE_DIR}/generation_best_data/nn_theta_set_{i}.npz"
        )
